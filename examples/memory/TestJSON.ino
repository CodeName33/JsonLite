...
void listObject(const String &str, const CJsonMemoryParserLite& value);

void listArray(const CJsonMemoryParserLite& item)
{
    if (item.isArray())
    {
...
const char jsonData[] = "{\"effects\":[{\"text\": \"FFT\",\"params\": [{\"name\": \"color\" ,\"type\": \"list\" ,\"list\": [{ \"text\": \"Неон\" },{ \"text\": \"Синий\" },5]}]}]}";
...
void setup() {
    ...
    CJsonMemoryParserLite parser((const char*)jsonData);
    if (auto propEffects = parser.getProp(F("effects")))
    {
        propEffects.getArrayItems([](const CJsonMemoryParserLite& propEffect) {
            String text = propEffect.getProp(F("text")).toString();
            if (auto propParams = propEffect.getProp(F("params")))
            {
                propParams.getArrayItems([](const CJsonMemoryParserLite& propParam) {
                    ...
                    {
                        propList.getArrayItems([](const CJsonMemoryParserLite& propListItem) {
                            String value;
